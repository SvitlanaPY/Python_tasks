"""
NamedTupe (іменований кортеж) - це також клас, який містить незмінні дані, але у форматі кортежу,
...
Наша змінна буде також належати і до кортежу: print(isinstance(<назва змінної>, tuple))
...
(які записуються в ліст чи в строку) і це все потрібно буде десь зберегти, 
і як правило зберігають в змінній з такою ж назвою як назва класу
(імя змінної, яка стоїть зліва == ім"я класу, що стоїть справа: Author = namedtuple('Author', ...)
"""
from collections import namedtuple
Author1 = namedtuple('Author1', ['topic', 'name', 'language'])   # створюємо клас Author1 з переліком атрибутів і записали в змінну Author1(назва змінної такаж як назва класу)
author_1 = Author1("ООП частина2", "Ростислав", "Англ.")    # створюємо екземпляр класу з переліком значень атрибутів
# де author_1 - екземпляр класу Author1 з переліком значень атрибутів
...
Author_new = namedtuple('Author_new', 'topic name language')  # перелік імен атрибутів можна вказувати в лісті, чи в стрічці ('topic name language').
