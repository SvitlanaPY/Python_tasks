...
"""
Якщо ми спробуємо обійти наш екземпляр класу в циклі for:
for i in igor:
    print(i)
то отримаємо помилку: TypeError: 'Student' object is not iterable

Списки є ітерабельними об"єктами, тобто їх можна обійти в циклі for.
a = [1, 2, 3, 4, 5]
Ітерабельними є об"єкти, які:
1. дають можливість обходити свої елементи по черзі, і 
2. можуть бути перетворені до ітератора за допомогою ф-ії iter(): b = iter(a)
print(b)   # <list_iterator object at 0x7f1ad0f7b2e0>
Ітератор представляє собою колекцію, елементи якої можна по-черзі обійти лише ОДИН раз, а обхід відпувається шляхом виклику ф-ії next() 
