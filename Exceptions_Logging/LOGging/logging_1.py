...
HANDLER-и - фільтрують отримані від логера повідомлення(LogRecord-об"єкти)
...
який створить строку потрібного/заданого нам вигляду, поверне строку хендлеру, який виведе її назовні.
...
formatter - це об"єкт, який відповідає за шаблон/темплейт виводу логів.
...
# ф-ія/метод getLogger() приймає один параметр - name, ім"я логера.
...
print()
my_logger_withName = logging.getLogger("SvP")
print("my_logger_withName: ", my_logger_withName)
# <Logger SvP (WARNING)>   - екземпляр класу RootLogger з іменем SvP
print()
...
print("my_logger.level: ", my_logger.level)
# my_logger.level:  30
print("my_logger_withName.level: ", my_logger_withName.level)
# my_logger_withName.level:  0
...
def main(name):
    my_logger.warning(f"MSG: Enter in the main() function: name = {name}")
#   у момент виклику метода warning(), логер створив екземпляр класу LogRecord
...
"""
Ф-ія getLogger() може приймати в якості аргументу ім"я (єдиний її аргумент).
Якщо ім"я не передано, то ф-ія getLogger() створює кореневий логер (root-логер) - RootLogger-об"єкт, якщо його немає.
А якщо він був десь створений, то вона повертає цей вже створений об"єкт.
Якщо ім"я було передано, то ф-ія getLogger() створить іменований логер, або поверне вже створений
власне, можна створювати і налаштовувати кастомні логери (logging_3.py)
"""
